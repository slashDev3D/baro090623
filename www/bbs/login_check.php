...
$loginWho    = isset($_POST['loginWho']) ? trim($_POST['loginWho']) : '';   // 로그인하는 대상이 누구인지 체크
...
// 로그인 대상과 레벨이 맞는지
if($loginWho == "loginStudent" && ($mb['mb_level'] >= '3' && $mb['mb_level'] <= '9' ))
{
    alert("학생 아이디로 다시 로그인해주세요");
}
else if ($loginWho == "loginInstructor" && $mb['mb_level'] <= '2')
{
    alert("인스트럭터 아이디로 다시 로그인해주세요");
}
...
if ( is_use_email_certify() && !preg_match("/[1-9]/", $mb['mb_email_certify'])) {
    ...
    confirm("{$mb['mb_email']} 메일로 메일인증을 받으셔야 로그인 가능합니다. 인증메일 재전송을 원하신다면, 취소를 클릭하시기 바랍니다.", G5_URL, G5_BBS_URL.'/register_email.php?mb_id='.$mb_id.'&ckey='.$ckey);
}
