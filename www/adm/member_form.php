...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        <th scope="row"><label for="mb_birth">생일</label></th>
        <td><input type="text" name="mb_birth" value="<?php echo $mb['mb_birth'] ?>" id="mb_birth" required class="required frm_input" size="15"  maxlength="20"></td>
	...
	<?php if($mb['mb_level']==2 || $mb['mb_level']==3 ){?>
    <tr>
        <th scope="row"><label for="mb_level">혁신단명</label></th>
        <td>
			...
			?>
			<option <?=$row['og_id']==$mb['mb_1']?"selected=true":""?> value="<?=$row['og_id']?>"><?=$row['og_name']?></option>
			<?php }?>
			</select> 
		</td>
    <?php } ?>
    <?php if($mb['mb_level']==2 ){?>
        <th scope="row"><label for="mb_level">팀명</label></th>
        <td>
			<select name="mb_2" id="mb_2" class="iptSt01">
				<option>팀명을 선택해주세요.</option>
				<?php
					$sql = "select * from g5_organize_team where og_id = {$mb['mb_1']}";
					$res = sql_query($sql);
					for ($i=0; $row=sql_fetch_array($res); $i++) {
				?>
				<option <?=$row['ogt_id']==$mb['mb_2']?"selected=true":""?> value="<?=$row['ogt_id']?>"><?=$row['ogt_name']?></option>
				<?php }?>
			</select> 
		</td>
    </tr>
    ...
    <tr>
        <th scope="row">
            <label for="mb_<?php echo $i ?>">
                <?php if($i == 1) { echo "혁신단명"; } else if($i == 2) { echo "팀명"; } else if($i == 3) { echo "직책"; } else { echo "여분필드 ".$i; }?>
            </label>
        </th>
        <td colspan="3">
            <input type="text" name="mb_<?php echo $i ?>" value="<?php echo $mb['mb_'.$i] ?>" id="mb_<?php echo $i ?>" class="frm_input" size="30" maxlength="255">
        </td>
    </tr>
    <?php } ?>
