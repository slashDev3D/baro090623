...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td headers="mb_list_mb1">
            <?php if($row['mb_level'] == 2 || $row['mb_level'] == 3 ) { ?>
                <select name="mb_1[<?php echo $i; ?>]" id="mb_1_<?php echo $i; ?>" onchange="getTeamInfo<?php echo $i; ?>(this.value)" class="iptSt01">
                    ...
                    ?>
                    <option <?=$dd['og_id']==$row['mb_1']?"selected=true":""?> value="<?=$dd['og_id']?>"><?=$dd['og_name']?></option>
                    <?php }?>
                </select>
            <?php } else { echo "-"; }?>
        </td>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td headers="mb_list_mb2">
            <?php if($row['mb_level'] == 2) { ?>
                <select name="mb_2[<?php echo $i; ?>]" id="mb_2_<?php echo $i; ?>" class="iptSt01">
                    <option>팀명을 선택해주세요.</option>
                        <?php
                            $sql = "select * from g5_organize_team where og_id = {$row['mb_1']}";
                            $res = sql_query($sql);
                            for ($k=0; $dd=sql_fetch_array($res); $k++) {
                        ?>
                            <option <?=$dd['ogt_id']==$row['mb_2']?"selected=true":""?> value="<?=$dd['ogt_id']?>"><?=$dd['ogt_name']?></option>
                        <?php }?>
                </select>
                
                <script>
                    function getTeamInfo<?php echo $i; ?> ($val){
                        $.ajax({
                            url:"/bbs/ajax.team.php",
                            type:"get",
                            data:{"og_id":$val},
                            success:function(res){
                                $("#mb_2_<?php echo $i; ?>").html(res);
                            }
                        });
                    }
                </script>
